...
- Surfaces of kind `platform` are one-way: they only stop a fall onto their top edge, never a rise and never a sideways move. Floors and cover are solid from every side. Shots already ignore platforms, so anything new that collides with the arena has to make the same distinction.
...
- Player artwork is a spritesheet per outfit under `public/art/sprites/`, a strip of four frames in the order given by `PLAYER_FRAMES` in `src/client/sprite-animation.ts`. Both children use the same character; role is shown by the name and the coloured pad, never by the drawing. Frame choice stays a pure function of authoritative state plus a wall clock, so it can be tested without a canvas.
- When a generated sheet needs slicing, find the drawings by their own outline rather than assuming a grid, and scale every frame by one shared factor. A generator places poses unevenly and adds shadows it was told not to draw; per-frame scaling would make the character change size while it walks.
