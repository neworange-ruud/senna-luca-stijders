...
- Overall status: Feature complete and deployed; the children have now played on their own iPads, and what that first play exposed is fixed and waiting to be deployed: two layout and input defects, the two changes the owner asked for afterwards (icons on every control and opening a chest by tapping it), and a second round of device feedback (browser zoom locked out, the iOS share callout refused, a margin around the arena, and the controls moved clear of a broken strip of touchscreen)
...
- Next action: Deploy the two rounds of iPad fixes together with the icons and chest tapping, then confirm on the devices that a held control no longer selects text, that the arena reaches the bottom of the screen, that a child finds a chest by tapping it, that no pinch or double tap changes the scale any more, that no share callout appears during a fight, and that every control on the iPad with the dead right-hand strip answers a finger. After that, pair the two iPads with the production pin and run the observed first-use and full-match journeys, recording the device identifiers, time to start, comprehension, multi-touch, frame rate, and latency. Whether the icons actually help a pre-reader is a question only the children can answer, so it belongs in that observation.
...
| --- | --- | --- |
| 2026-08-22 | The page is locked at one scale. The viewport is pinned at both ends, `touch-action` on the root allows panning but not zooming, and Safari's own pinch gestures, a ctrl-wheel pinch and the keyboard zoom shortcuts are all refused. Every focusable field is at least 16 px, because a smaller one makes Safari zoom in on focus and never zoom back out. | Requested by the owner after the second play. The arena is drawn to fill the screen it is given; once it is zoomed, a child sees a corner of the world and has no idea how to undo that, and neither the fight nor the controls survive it. Panning stays allowed because the lobby is taller than a screen. |
| 2026-08-22 | Nothing outside a text field may be selected, no picture may be long-pressed into an offer, and any selection that appears anyway is taken away again. Touches on the arena are refused outright, except on the few controls in there that still need a tap turned into a click. | Requested by the owner: a "Deel..." button kept appearing mid-fight. That is the iOS callout bar, which appears both for a selection and for a picture under a resting finger, and a canvas counts as a picture. Refusing the gesture is not enough on its own, so the selection is also cleared after the fact. Touches are refused only inside the stage, because a refused touch is also a page that will not scroll. |
| 2026-08-22 | The row of controls keeps two buttons' width clear of both the left and the right edge of the screen. | One of the two iPads has a strip down its right-hand side that no longer answers a finger, and replacing it is not on the table. Moving the controls inward costs nothing but sky and makes the fault invisible. Both ends are inset rather than only the right, because one cluster against its edge and one away from it reads as a mistake rather than a decision. |
| 2026-08-21 | A chest is opened by tapping the chest itself. The Action button keeps working and stays the way to ride a lift or pick a weapon off the ground. | Requested by the owner. A child who cannot read was hunting for a named button while the thing they wanted was right in front of them. The tap only raises the same Action control the button raises, so the room still judges every claim; reach is checked before raising it, so a tap meant for a chest can never ride a lift by accident. The button stays because a lift and a dropped weapon have nothing on screen to aim at, and because it is the only way to open a chest with a keyboard or a screen reader. |
| 2026-08-21 | Draw the control icons as inline SVG in the page instead of generating them through the Layer MCP like the rest of the artwork. | The icon is the whole point of the change: for a child who cannot read, a button whose picture failed to load carries no meaning at all, which is exactly what the missing-asset fallback allows for game art. Inline drawings also stay sharp at any button size, follow the button colour, and cost nothing to load. Generated PNG pictograms would have been blurry at 27 pixels and inconsistent with each other. |
...
| 2026-08-21 | Pre-reader controls | Focused chest tap suite | Passed: 6 tests covering the chest a tap means, the tolerance around it, the nearer of two overlapping chests, a tap aimed at nothing, and that the reach a tap needs is the same reach the authoritative claim grants, including refusing a knocked-out child standing on top of the chest | `tests/unit/chests.test.ts` |
| 2026-08-21 | Pre-reader controls | Chest tap journey in Chromium and WebKit | Passed: with a chest landed in reach, one short tap on the chest turned Luca's weapon into the sword the chest held, a tap on empty arena 460 pixels away changed nothing, and the opponent's device agreed | `tests/e2e/chests.spec.ts`, `docs/checkpoints/phase-10-chest-tap.png` |
| 2026-08-21 | Pre-reader controls | Control icon legibility and naming | Passed: each of the seven controls carries a distinct silhouette, checked magnified at four times scale; the existing control audit still finds every button named and at least 44 by 44 pixels, and all five action buttons now sit on one row on an iPad in landscape | `docs/checkpoints/phase-10-ipad-fit.png`, `tests/e2e/accessibility.spec.ts` |
| 2026-08-21 | Pre-reader controls | `npm run check`, `npm run test:e2e`, `npm run test:e2e:webkit` | Passed: 270 unit, 10 Node, 14 Worker tests, formatting, typed lint, three-runtime type checks, production build, and 31 Chromium plus 29 WebKit journeys with the 2 pre-existing WebKit skips | Local command output |
| 2026-08-22 | Second device round | Browser zoom lockout at 1024x768 in both engines | Passed: the viewport is pinned at `minimum-scale=1.0`, `maximum-scale=1.0` and `user-scalable=no`; `gesturestart`, `gesturechange` and `gestureend` are all refused on the arena, as are a ctrl-wheel pinch and ctrl with plus, minus and zero; the body reports `touch-action: pan-x pan-y`, so two fingers can pan but never zoom | `tests/e2e/ipad.spec.ts` |
| 2026-08-22 | Second device round | iOS share callout checks | Passed: a touch starting on the arena is refused outright, which is what stops iOS offering to save or share the canvas; a touch on the pause button is still allowed through so its tap becomes a click; a selection made on the status bar is gone again within 50 ms; and a grown-up selecting all four digits of the pincode keeps that selection | `tests/e2e/ipad.spec.ts` |
| 2026-08-22 | Second device round | Arena margin and centring at 1024x768 | Passed with the test panel taken away, as in a real match: at least 20 px of sky on all four sides, and the left and right gaps and the top and bottom gaps each equal to within 1 px | `tests/e2e/ipad.spec.ts`, `docs/checkpoints/phase-10-ipad-fit.png` |
| 2026-08-22 | Second device round | Controls clear of the screen edges | Passed: the nearest control is at least two button widths from both the left and the right edge of the screen, and all seven still sit on one row on an iPad in landscape | `tests/e2e/ipad.spec.ts` |
| 2026-08-22 | Second device round | `npm run check`, `npm run test:e2e`, `npm run test:e2e:webkit` | Passed: 270 unit, 10 Node, 14 Worker tests, formatting, typed lint, three-runtime type checks, production build, and 35 Chromium plus 33 WebKit journeys with the 2 pre-existing WebKit skips | Local command output |
...
| 2026-08-22 | Second device round | Closed every route to a browser zoom, refused the iOS offer to share whatever a resting finger lands on, framed the arena with a margin and centred it, and pulled the controls two buttons clear of both screen edges so the iPad with a dead strip of touchscreen down its right-hand side still answers every press. | Complete and verified. `npm run check` (270 unit, 10 Node, 14 Worker), 35 Chromium and 33 WebKit journeys pass, including four new iPad-sized regressions. Restart at Phase 10's first unchecked task; inspect `docs/checkpoints/phase-10-ipad-fit.png`. |
| 2026-08-21 | Pre-reader controls | Gave every control a drawn icon beside its Dutch word, so a child who cannot read yet recognises the buttons by their picture, and made a chest open by tapping the chest itself rather than by finding the Action button. The tap raises the same Action control the button raises and checks the same reach the authoritative claim uses, so nothing about who may open what has moved into the browser. A ring around a chest in reach and the tip above the arena now appear at exactly the moment a tap would work. | Complete and verified. `npm run check` (270 unit, 10 Node, 14 Worker), 31 Chromium and 29 WebKit journeys pass, including a new chest tap journey in both engines. Restart at Phase 10's first unchecked task; inspect `docs/checkpoints/phase-10-chest-tap.png`. |
